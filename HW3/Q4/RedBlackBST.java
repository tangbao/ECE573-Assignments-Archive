...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    /**
     * Get the number of red nodes in a red-black bst
     */

    public int getNumOfRedNodes(){
        return getNumOfRedNodes(root);
    }

    private int getNumOfRedNodes(Node node){
        int cnt = 0;
        if(node == null){
            return cnt;
        }
        cnt = getNumOfRedNodes(node.left) + getNumOfRedNodes(node.right);
        if(isRed(node)){
            cnt++;
        }
        return cnt;
    ...
    private int getInternalPathLen(Node node){
        ...
        pathLen = node.size + getInternalPathLen(node.left) + getInternalPathLen(node.right);
        return pathLen;
    }
